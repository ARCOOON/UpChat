...
public class SketchwareUtil {
    ...
    public static void CustomToast(Context _context, String _message, int _textColor, int _textSize, int _bgColor, int _radius, int _gravity) {
        ...
        switch (_gravity) {
            case 1 -> _toast.setGravity(Gravity.TOP, 0, 150);
            case 2 -> _toast.setGravity(Gravity.CENTER, 0, 0);
            case 3 -> _toast.setGravity(Gravity.BOTTOM, 0, 150);
        }
        _toast.show();
    }
    ...
    public static void sortListMap(final ArrayList<HashMap<String, Object>> listMap, final String key, final boolean isNumber, final boolean ascending) {
        Collections.sort(listMap, (compareMap1, compareMap2) -> {
            if (isNumber) {
                int _count1 = Integer.valueOf(compareMap1.get(key).toString());
                int _count2 = Integer.valueOf(compareMap2.get(key).toString());
                if (ascending) {
    ...
    public static void CropImage(Activity activity, String path) {
        try {
            ...
            crop_intent.setDataAndType(contentUri, "image/*")
                    .putExtra("crop", "true")
                    .putExtra("aspectX", 1)
                    .putExtra("aspectY", 1)
            ...
            activity.startActivityForResult(crop_intent, 1);
